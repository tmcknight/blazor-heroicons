...
internal class RegistryGenerator(string rootPath)
{
    public void GenerateHeroiconRegistry(IReadOnlyList<(string iconType, string svgDir)> iconSets)
    {
        ...
        writer.WriteLine("{");
        writer.WriteLine("\tprivate const DynamicallyAccessedMemberTypes ComponentMembers = DynamicallyAccessedMemberTypes.PublicConstructors;");
        writer.WriteLine();
        ...
        writer.WriteLine("\t[return: DynamicallyAccessedMembers(ComponentMembers)]");
        writer.WriteLine("\t[UnconditionalSuppressMessage(\"Trimming\", \"IL2073\", Justification = \"All values are statically-typed via typeof() and preserved by the linker.\")]");
